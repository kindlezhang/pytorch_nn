...
class BasicBlock(nn.Module): # 定义一个BasicBlock类，继承nn.Module，适用于resnet18、34的残差结构
    ...
    def forward(self,x):
        ...
        if self.downsample is not None: # 如果下采样层不为空，则对输入进行下采样得到捷径分支的输出
...
class ResNet(nn.Module):  # 定义resnet网络的框架部分
    ...
    def _make_layer(self,block,channel,block_num,stride=1): # 创建一个残差层
        ...
        if stride != 1 or self.in_channel != channel*block.expansion:
            # 对于layer1的构建 使用resnet18(expension = 1) 不满足条件 会跳过下采样的操作
            # 对于resnet50 101  满足条件 会进行下采样操作 通道数由64 变为256 需要对齐便于进行残差连接
            # stride为2时 空间尺寸减半,因为从1到2,2到3， 3到4的每个layer的第一个残差结构的主分支的第一个卷积核的stride都是2
            # 进行下采样，不然连接不上
        ...
        # *意思是把列表 layers 中的元素逐一传入 nn.Sequential 的多个参数中
    ...
    def forward(self,x):
        ...
        # include_top=True：网络包含最后的 全连接层 (fc) 和池化层 (avgpool)，输出类别概率。
        # 用于图像分类任务。

        # include_top=False：只保留 卷积特征提取部分（backbone），不包含分类层。
        # 用于特征提取、迁移学习或下游任务（如检测/分割）
